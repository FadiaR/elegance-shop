...
export function useStore() {
  ...
  const recordSale = useCallback((productId: string, quantity: number, actualPrice: number) => {
    ...
    const sale: Sale = {
      ...
      brand: product.brand,
      quantity,
      purchasePrice: product.purchasePrice + (product.shippingCost || 0),
      listedPrice: product.sellingPrice,
      actualPrice,
      potentialProfit: (product.sellingPrice - product.purchasePrice - (product.shippingCost || 0)) * quantity,
      realProfit: (actualPrice - product.purchasePrice - (product.shippingCost || 0)) * quantity,
      soldBy: userName,
      createdAt: new Date().toISOString(),
    };
